...
SOUNDS["die"] = mixer.Sound(cur_path + "/assets/audio/die.ogg")
SOUNDS["wing"] = mixer.Sound(cur_path + "/assets/audio/wing.ogg")
SOUNDS["point"] = mixer.Sound(cur_path + "/assets/audio/point.mp3")
SOUNDS["hit"] = mixer.Sound(cur_path + "/assets/audio/hit.mp3")
SOUNDS["background"] = mixer.Sound(cur_path + "/assets/audio/background.wav")

SOUNDS["background"].set_volume(0.9)
SOUNDS["wing"].set_volume(0.5)
SOUNDS["hit"].set_volume(0.5)
SOUNDS["die"].set_volume(0.5)
SOUNDS["point"].set_volume(0.5)
...
def init_texture():
    # ################### Pipe Textures ###########################################################
    img_pipe_load = image.load(cur_path + '/assets/sprites/pipe.png')
    ...
    TEXTURES["bird"] = tex
    # ################### Background Texture ############################################################
    img_backG_load = image.load(cur_path + '/assets/sprites/background.png')
    ...
    # ################### base Texture ############################################################
    img_base_load = image.load(cur_path + '/assets/sprites/ground.png')
    ...
    TEXTURES["base"] = tex
    # ################### message Texture ############################################################
    img_msg_load = image.load(cur_path + '/assets/sprites/ready.png')
...
def keyboard(key, a, b):
    # TODO: Jump and jump duration are deleted
...
def show_game_over():
    draw_rectangle_with_tex(100, 500, 150, 850, TEXTURES["game over"], 0.9)
    draw_rectangle_with_tex(0, SCREENWIDTH, 0, BASEY + 200, TEXTURES["restart"], 1.5)


def show_welcome():
    draw_rectangle_with_tex(80, 520, 250, 790, TEXTURES["msg"], 0.3)
    draw_rectangle_with_tex(0, SCREENWIDTH, 0, BASEY + 200, TEXTURES["start"], 0.5)
...
def display():
    ...
    # bird.bottom = current_mouse_y - 17.5
    # bird.top = current_mouse_y + 17.5
